...
```cfscript
component {
	property name="listmonk" inject="ListmonkClient@listmonk";
...
Module settings `subscriberMode` and `contentType` are applied automatically when those keys are omitted from the payload. Pass `subscriber_mode` / `content_type` on the payload to override.
...
# Start BoxLang test harness server
box run-script start:boxlang

# Build zip + DocBox API docs
box task run build/Build.cfc projectName=listmonk version=0.1.0
```
